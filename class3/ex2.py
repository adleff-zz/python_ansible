...
The first image file should graph the input and output octets on interface
FA4 on pynet-rtr1 every five minutes for an hour.
Use the pygal library to create the SVG graph file.
Note, you should be doing a subtraction here
(i.e. the input/output octets transmitted during this five minute interval).

The second SVG graph file should be the same as the first except
graph the unicast packets received and transmitted.
...
def main():
    '''
    Main function uses a list for each OID type to hold polled interface
    stats every 5 minute interval.  There is also a corresponding list
    for each OID type used to keep track of the previous polled interface 
    total, since the router increments the OID data starting at uptime.
    The base total is subtracting from the current interface total in order
    to display the incremental interface usage during a 5 minute interval.
    '''
    ...
    for interval in range(0, 65, 5):
        ...
        for interface_counter,oid in interface_oids:
            ...
            if interface_counter is 'ifInOctets_fa4':
                current_counter = (snmp_extract(snmp_get_oid_v3(pynet_rtr1, snmp_user, oid)))
